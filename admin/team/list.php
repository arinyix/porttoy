...
.table.rtable img.avatar{ width:56px; height:56px; object-fit:cover; border-radius:999px; border:1px solid #e7e7e7; background:#fff; }
...
.dark .table.rtable img.avatar{ border-color:var(--card-border); background:#000; }
...
<table class="table rtable fade-in">
  <thead>
    <tr>
      <th>#</th><th>Foto</th><th>Nome</th><th>Função</th><th>Lattes</th><th style="width:260px">Ações</th>
    </tr>
  </thead>
  <tbody>
    ...
    <tr>
      ...
      <td data-label="Foto">
        <?php if (!empty($r['photo_path'])): ?>
          <img class="avatar" src="<?= e(base_url($r['photo_path'])) ?>" alt="Foto de <?= e($r['name']) ?>">
        <?php else: ?>—<?php endif; ?>
      </td>
      <td data-label="Nome"><?= e($r['name']) ?></td>
      <td data-label="Função"><?= e($r['role']) ?></td>
      <td data-label="Lattes">
      ...
      <td data-label="Ações">
        <div class="actions-row">
          ...
          <form action="<?= e(base_url('admin/team/delete.php')) ?>" method="post"
  ...
  <?php endforeach; ?>
  <?php if (!$rows): ?><tr><td colspan="6">Nenhum membro cadastrado.</td></tr><?php endif; ?>
  </tbody>
</table>
